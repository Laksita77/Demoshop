...
const FROM_ADDRESS = process.env.EMAIL_FROM_NAME
  ? `${process.env.EMAIL_FROM_NAME} <onboarding@resend.dev>`
  : "QA AI Pipeline <onboarding@resend.dev>";
...
async function sendApprovalEmail({ title, category, expected, actual, reason, fixes, testCase, approveUrl, declineUrl }) {
  ...
  try {
    ...
    console.error(`      Resend → from:${FROM_ADDRESS} to:${to}`);
  }
}
...
<p style="color:#888;font-size:12px;">Sent at ${new Date().toLocaleString()}</p>
  ...
  try {
    await resend.emails.send({ from: FROM_ADDRESS, to: [to], subject: "✅ DemoShop Email Test — Working", html });
